...
#region
    ...
    public static class PatchStartCarryThingDelegate
    {
        ...
        private static MethodBase TargetMethod() //The target method is found using the custom logic defined here
        {
            ...
            PatchStartCarryThingDelegate._predicateClass = typeof(Toils_Haul).GetNestedTypes(AccessTools.all).FirstOrDefault(t => t.FullName.Contains("c__DisplayClass1_0"));
            ...
            var m = PatchStartCarryThingDelegate._predicateClass.GetMethods(AccessTools.all).FirstOrDefault(t => t.Name.Contains("b__0"));

            if (m == null) { Log.Error("LWM.Deep_Storage: Could not find Verse.AI.Toils_Haul:c__AnonStorey0<>m__0"); }
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (; i < code.Count - 1; i++)
            {
                ...
                if (code[i].opcode == OpCodes.Brtrue || code[i].opcode == OpCodes.Brtrue_S)
                {
                    // keep track of where Branch on True are going
                    branchLabel = (Label) code[i].operand;
                }
                else if (!foundCorrectException && code[i].opcode == OpCodes.Ldstr && (string) code[i].operand == "StartCarryThing got availableStackSpace ")
                {
                ...
                else if (foundCorrectException && code[i].opcode == OpCodes.Throw)
                {
                    ...
                    //   Brtrue jump goes to:
                    if (!code[i].labels.Remove(branchLabel)) { throw new SystemException("LWM.DeepStorage: Patching StartCarryThing failed: label failure"); }
                    ...
                    yield return new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(PatchStartCarryThingDelegate._predicateClass, "toil"));
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method("LWM.DeepStorage.Patch_StartCarryThing_Delegate:FillThisStackIfAble"));
            ...
            for (; i < code.Count - 1; i++) { yield return code[i]; }

            var cleanUp = new CodeInstruction(OpCodes.Call, AccessTools.Method("LWM.DeepStorage.Patch_StartCarryThing_Delegate:CleanUpStacks"))
            {
            ...
            {
                operand = code[i].operand, blocks = code[i].blocks
            };
        ...
        public static void FillThisStackIfAble(Toil toil, Thing thing, int carryCapacity)
        {
            ...
            if (slotGroup == null || !(slotGroup?.parent is ThingWithComps) || ((ThingWithComps) slotGroup.parent).TryGetComp<CompDeepStorage>() == null)
            {
            ...
            for (var i = 0; i < thingsHere.Count; i++)
            {
                ...
                if (otherThing.Spawned) { otherThing.Map.listerMergeables.Notify_ThingStackChanged(otherThing); }

                if (thing.Spawned) { thing.Map.listerMergeables.Notify_ThingStackChanged(thing); }
