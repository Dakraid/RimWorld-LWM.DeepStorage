...
#region
    ...
    {
        private static readonly Dictionary<string, object> _defaultDefValues = new Dictionary<string, object>();
        ...
        {
            if (DefChangeTracker._defaultDefValues.ContainsKey(defName + "_" + keylet)) { return (T) DefChangeTracker._defaultDefValues[defName + "_" + keylet]; }
        ...
        public void UpdateToNewValue<T>(string defName, string keylet, T value, ref T refToChange) where T : IComparable
        {
            ...
            var defaultValue = GetDefaultValue(defName, keylet, refToChange);
            refToChange = value;
        ...
        public IEnumerable<T> GetAllWithKeylet<T>(string keylet)
        {
            foreach (var entry in DefChangeTracker._defaultDefValues)
            {
                ...
                if (t[t.Length - 1] == keylet) { yield return (T) entry.Value; }
            }
        }
        ...
        public void ExposeSetting<T>(string defName, string keylet, ref T value) where T : IComparable
        {
            if (Scribe.mode == LoadSaveMode.LoadingVars)
            {
                ...
                {
                    if (DefChangeTracker._defaultDefValues.Remove(defName + "_" + keylet)) { Utils.Mess(Utils.Dbf.Settings, "  loaded default value for " + defName + "'s " + keylet + " and removed record"); }
                }
            }
        ...
        public bool IsDefaultValue<T>(string defName, string keylet, T value) where T : IComparable
        {
            ...
            if (!DefChangeTracker._defaultDefValues.ContainsKey(key)) { return true; }
