...
namespace LWM.DeepStorage
{
    public class CellStorageCollection : ICollection<Thing>, IExposable
    {
        ...
        /// <remarks>
        ///     It is called in <see cref="PatchNotifyReceivedThing"/>. <see cref="Thing.stackCount"/> is sanitized in <see cref="GenSpawn.Spawn"/>.
        ...
        {
            if (!_cacheCell.TryGetValue(item.Position, out var model))
            {
                return;
            }
        ...
        public bool Remove(Thing item)
        {
            if (_thingsInStore.Remove(item)
                & (_cacheCell.TryGetValue(item.Position, out var model)
                   && model.Remove(item)))
            {
                return true;
            }
    ...
    #region Implementation of IExposable
        ...
        {
            if (_cacheCell.TryGetValue(thing.Position, out var model))
            {
                model.Update(thing);
            }
        }
        ...
        {
            if (!_cacheCell.TryGetValue(loc, out var cellStorage))
            {
                return false;
            }

            if (!cellStorage.NonFullThings.TryGetValue(thing, out var nonFullThing))
            {
                return false;
            }
