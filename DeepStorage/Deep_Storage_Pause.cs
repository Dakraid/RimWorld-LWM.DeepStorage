...
#region
    ...
    public static class PatchPlaceHauledThingInCellToil
    {
        ...
        {
            Utils.Warn(Utils.Dbf.PlaceHauledThingInCell, "Starting new haul job, toils created");
            ...
            result.initAction = delegate
            {
                ...
                Utils.Warn(Utils.Dbf.PlaceHauledThingInCell, "initAction called for " + actor + "'s haul job " + curJob.def.driverClass + " to " + cell + " (toil " + actor.jobs.curDriver.CurToilIndex + ")");
                ...
                if (!(slotGroup?.parent is ThingWithComps) || (cds = ((ThingWithComps) slotGroup?.parent)?.GetComp<CompDeepStorage>()) == null)
                {
                    Utils.Warn(Utils.Dbf.PlaceHauledThingInCell, "not going into Deep Storage");
                ...
                if (timeStoringTakes <= 1 || !Settings._storingTakesTime) //boo, hiss, but some ppl use it
                {
                    // just like vanilla
                    Utils.Warn(Utils.Dbf.PlaceHauledThingInCell, "Instantaneous storing time");
                ...
                Utils.Mess(Utils.Dbf.PlaceHauledThingInCell, "  Storing time set to: " + actor.jobs.curDriver.ticksLeftThisToil);
                // It'd be nice to have a progress bar for deep storage
                result.WithProgressBar(TargetIndex.B, () => 1f - (float) result.actor.jobs.curDriver.ticksLeftThisToil / timeStoringTakes, true);
            ...
            result.tickAction = delegate
            {
                ...
                {
                    Utils.Warn(
                        Utils.Dbf.PlaceHauledThingInCell,
                        "  "
                        + pawn
                        + " hit "
                        + pawn.jobs.curDriver.ticksLeftThisToil
                        + " ticks; about to put down "
                        + (pawn.carryTracker.CarriedThing != null ? "" + pawn.carryTracker.CarriedThing.stackCount + pawn.carryTracker.CarriedThing : "NULL ITEM")
                    );
