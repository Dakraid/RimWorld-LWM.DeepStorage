...
#region
    ...
    internal class PatchHaulToCellStorageJob
    {
        ...
        public static IEnumerable<CodeInstruction> XTranspiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator) {
            ...
            var callThingAt = AccessTools.Method(
                typeof(ThingGrid), "ThingAt", new[]
                {
                    typeof(IntVec3), typeof(ThingDef)
                }
            );
            var inDeepStorage = generator.DeclareLocal(typeof(bool)); // new local variable

            var checkedInDeepStorage = false;
            ...
            for (var i = 0; i < code.Count; i++)
            {
                ...
                if (code[i].opcode == OpCodes.Stloc_1 && checkedInDeepStorage == false)
                {
                    // slotGroup
                    //Log.Warning("About to return Stloc_1: i is " + i);
                    yield return code[i++]; // the Stloc.1 we just found

                    if (AccessTools.Method("LWM.DeepStorage.Utils:CanStoreMoreThanOneThingIn") == null) { Log.Error("Failure, cannot find CSMTOTI"); }

                    yield return new CodeInstruction(OpCodes.Ldloc_1); // slotGroup
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method("LWM.DeepStorage.Utils:CanStoreMoreThanOneThingIn"));
                ...
                if (code[i].opcode == OpCodes.Ldarg_0
                    ...
                    (MethodInfo) code[i + 1].operand == AccessTools.Method(typeof(Thing), "get_Map")
                    && // p.Map
                    code[i + 2].opcode == OpCodes.Ldfld
                    && (FieldInfo) code[i + 2].operand == AccessTools.Field(typeof(Map), "thingGrid"))
                {
                    // p.Map.thingGrid...
                    ...
                    yield return c;
                    var vanillaThingAt = generator.DefineLabel();
                    ...
                    for (; j < code.Count; j++)
                    {
                        // this loads the cell and the def
                        if (code[j].opcode == OpCodes.Callvirt && (MethodInfo) code[j].operand == callThingAt)
                        {
                            // skip ThingAt()
                    ...
                    // insert our call
                    yield return new CodeInstruction(OpCodes.Callvirt, AccessTools.Method("LWM.DeepStorage.Patch_HaulToCellStorageJob:NullOrLastThingAt"));

                    for (j++; j < code.Count; j++)
                    {
                        // skip the ThingAt call, keep going to branch on false:
                    ...
                    var backToNormal = generator.DefineLabel();
                ...
                if (code[i].opcode == OpCodes.Stloc_3 && code[i - 1].opcode == OpCodes.Add)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method("LWM.DeepStorage.Patch_HaulToCellStorageJob:LogNum"));
                }
            #endif
            }
        } // end Transpiler
        ...
        public static Thing NullOrLastThingAt(Map map, IntVec3 c, ThingDef def)
        {
            ...
            Utils.Err(Utils.Dbf.HaulToCellStorageJob, "  testing for def " + def + " at " + c + "; " + freeSlots + " slots.");
            ...
            for (var i = 0; i < l.Count; i++)
            {
                ...
                freeSlots--;
                Utils.Mess(Utils.Dbf.HaulToCellStorageJob, "  Checking item " + l[i] + "; now have " + freeSlots + " left.");
                ...
                {
                    if (l[i].stackCount <= lastThing.stackCount) { lastThing = l[i]; }
                }
            }

            if (freeSlots > 0)
            {
                Utils.Err(Utils.Dbf.HaulToCellStorageJob, "  Final count of free slots: " + freeSlots);

                return null;
            }
            Utils.Warn(Utils.Dbf.HaulToCellStorageJob, "  Final item count: " + (lastThing == null ? "NULL" : lastThing.stackCount.ToString()));
        ...
        public static bool Prefix(Pawn p, Thing t, IntVec3 storeCell, ref Job result)
        {
            if (Utils.GetDeepStorageOnCell(storeCell, p.Map, out var comp) && comp is CompCachedDeepStorage compCached)
            {
                var   capacity         = compCached.CapacityToStoreThingAt(t, p.Map, storeCell);
                float carryingCapacity = p.carryTracker.AvailableStackSpace(t.def);

                var job = JobMaker.MakeJob(JobDefOf.HaulToCell, t, storeCell);
        ...
        {
            Utils.Warn(Utils.Dbf.HaulToCellStorageJob, "----" + line + ": " + num + " vs " + count + " (total max: " + statValue + ")");

            if (t1 != null) { Utils.Mess(Utils.Dbf.HaulToCellStorageJob, "        t1 is " + t1.stackCount + " " + t1); }

            if (t2 != null) { Utils.Mess(Utils.Dbf.HaulToCellStorageJob, "        t2 is " + t2.stackCount + " " + t2); }
        }

        public static void Postfix(Job result) => Utils.Err(Utils.Dbf.HaulToCellStorageJob, "Final Job size is " + result.count);
        ...
        {
            Utils.Err(Utils.Dbf.HaulToCellStorageJob, "Job request for " + t.stackCount + t + " by pawn " + p + " to " + storeCell);
