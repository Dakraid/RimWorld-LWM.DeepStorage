 ...
 */
    ...
    internal static class PatchTryFindBestBetterStoreCellFor
    {
        [UsedImplicitly]
        private static bool Prepare()
        {
            Utils.Mess(Utils.Dbf.Settings, "Patch to check if overCapacity? " + Settings._checkOverCapacity);

            if (!Settings._checkOverCapacity) { return false; }
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            var overCapacity = generator.DeclareLocal(typeof(bool));
            var i            = 0;
            ...
            for (; i < code.Count; i++)
            {
                ...
                if (code[i].opcode != OpCodes.Stloc_1) { continue; }
                ...
                yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(PatchTryFindBestBetterStoreCellFor), "OverCapacity"));
            ...
            */
            for (; i < code.Count; i++)
            {
                // Go until find the storage priority for slotGroup (priority) and StLoc it:
                if (code[i].opcode == OpCodes.Stloc_S && ((LocalBuilder) code[i].operand).LocalIndex == 7) { break; }
            ...
            var doNotBreak = generator.DefineLabel();

            for (; i < code.Count; i++)
            {
                if (code[i].opcode == OpCodes.Ldloc_S && ((LocalBuilder) code[i].operand).LocalIndex == 7 && code[i + 1].opcode == OpCodes.Ldarg_3 && code[i + 2].opcode == OpCodes.Ble_S)
                {
        ...
        [UsedImplicitly]
        private static bool OverCapacity(Map map, Thing thing, ref StoragePriority storagePriority)
        {
            Utils.Warn(
                Utils.Dbf.ShouldRemoveFromStorage,
                "LWM.DeepStorage: Over Capacity Called." + (map == null ? "  Map is NULL (this is bad)." : "  Map: " + map) + (thing == null ? "  Thing is NULL (this is bad)." : "  Thing: " + thing.stackCount + thing)
            );

            if (!thing.Spawned) { return false; }

            if (storagePriority == StoragePriority.Unstored) { return false; }
            var         cds = (thing.Position.GetSlotGroup(map)?.parent as ThingWithComps)?.GetComp<CompDeepStorage>();
            List<Thing> l;
            ...
            if (cds == null)
            {
                ...
                foreach (var t in l)
                {
                    Utils.Mess(Utils.Dbf.ShouldRemoveFromStorage, "LWM.DeepStorage: Not in DSU: looking at item " + (t != null ? t.ToString() : "NULL ITEM"));

                    if (!t.def.EverStorable(false)) { continue; }

                    if (thing == t) // first thing is okay.
                    {
                        return false;
                    }

                    Utils.Warn(Utils.Dbf.ShouldRemoveFromStorage, "LWM.DeepStorage: " + thing.stackCount + thing + " is not in a DSU and there is already a thing at " + thing.Position);
            ...
            if (cds._limitingFactorForItem > 0f)
            {
                if (thing.GetStatValue(cds._stat) > cds._limitingFactorForItem)
                {
                    Utils.Warn(Utils.Dbf.ShouldRemoveFromStorage, "LWM.DeepStorage: " + thing.stackCount + thing + " is too heavy for DSU at " + thing.Position);
                    storagePriority = StoragePriority.Unstored;
            ...
            Utils.Mess(Utils.Dbf.ShouldRemoveFromStorage, "  Checking ThingListAt now...");
            l = map.thingGrid.ThingsListAt(thing.Position);

            foreach (var thingInStorage in l)
            {
                ...
                if (!thingInStorage.def.EverStorable(false)) { continue; }

                // an "item" we care about
                Utils.Mess(Utils.Dbf.ShouldRemoveFromStorage, "    Checking Item " + thingInStorage.stackCount + thingInStorage);
                stacksStoredHere++;

                if (cds._limitingTotalFactorForCell > 0f)
                {
                    totalWeightStoredHere += thingInStorage.GetStatValue(cds._stat) * thingInStorage.stackCount;
                    Utils.Mess(Utils.Dbf.ShouldRemoveFromStorage, "      total mass so far: " + totalWeightStoredHere + " / " + cds._limitingTotalFactorForCell);

                    if (totalWeightStoredHere > cds._limitingTotalFactorForCell && stacksStoredHere >= cds.MinNumberStacks)
                    {
                        Utils.Warn(Utils.Dbf.ShouldRemoveFromStorage, "LWM.DeepStorage: " + thing.stackCount + thing + " is over weight capacity at " + thing.Position);
                        storagePriority = StoragePriority.Unstored;
                ...
                if (stacksStoredHere < cds.MaxNumberStacks) { continue; }

                // breaks if minNumberStacks > maxNumberStacks. I'm okay with this
                Utils.Warn(Utils.Dbf.ShouldRemoveFromStorage, "LWM.DeepStorage: " + thing.stackCount + thing + " is over capacity at " + thing.Position);
                storagePriority = StoragePriority.Unstored;
