 ...
 */
    ...
    internal static class PatchTryFindBestBetterStoreCellFor {
        private static bool Prepare() {
            ...
            if (!Settings._checkOverCapacity)
            {
                return false;
            }
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions,
            ...
            for (; i<code.Count;i++) {
                ...
                if (code[i].opcode==OpCodes.Stloc_1){ // StoragePriority storagePriority = currentPriority;
                    ...
                    yield return new CodeInstruction(OpCodes.Call,
                                                     HarmonyLib.AccessTools.Method(typeof(PatchTryFindBestBetterStoreCellFor),"OverCapacity"));
            ...
            if (i==code.Count)
            {
                Log.Error("LWM.DeepStorage: TryFindBestBetterStoreCellFor transpile failed(0)");
            }
            ...
            */
            ...
            if (i==code.Count)
            {
                Log.Error("LWM.DeepStorage: TryFindBestBetterStoreCellFor transpile failed(1)");
            }
            ...
            if (i==code.Count)
            {
                Log.Error("LWM.DeepStorage: TryFindBestBetterStoreCellFor transpile failed(2)");
            }
            ...
            foreach (var thingInStorage in l)
            {
                ...
                if (cds._limitingTotalFactorForCell > 0f) {
                    ...
                    Utils.Mess(ShouldRemoveFromStorage, "      total mass so far: "+totalWeightStoredHere+" / "+
                                                        cds._limitingTotalFactorForCell);
                    if (totalWeightStoredHere > cds._limitingTotalFactorForCell &&
