...
#region
    ...
    public class Utils
    {
        ...
        {
            Testing,
            NoStorageBlockerseIn,
            HaulToCellStorageJob,
            TryPlaceDirect,
            Spawn,
            TidyStacksOf,
            DeepStorageJob,
        ...
        public static HashSet<Thing> _topThingInDeepStorage = new HashSet<Thing>(); // for display

        // Nifty! Won't even be compiled into assembly if not DEBUG
        [Conditional("DEBUG")]
        public static void Warn(Dbf l, string s)
        {
            if (Utils._showDebug[(int) l]) { Log.Warning("LWM." + l + ": " + s); }
        }

        [Conditional("DEBUG")]
        public static void Err(Dbf l, string s)
        {
            if (Utils._showDebug[(int) l]) { Log.Error("LWM." + l + ": " + s); }
        }

        [Conditional("DEBUG")]
        public static void Mess(Dbf l, string s)
        {
            if (Utils._showDebug[(int) l]) { Log.Message("LWM." + l + ": " + s); }
        }
        ...
        public static bool CanStoreMoreThanOneThingAt(Map map, IntVec3 loc, Thing thing)
        {
            ...
            #pragma warning disable CS0162 // Unreachable code detected
                ...
                Log.Warning("Just for the record, " + (Scribe.mode == LoadSaveMode.LoadingVars) + (Scribe.mode == LoadSaveMode.PostLoadInit) + Scribe.mode);
            ...
            {
                var result = compCached.StorageSettings.AllowedToAccept(thing) && compCached.StackableAt(thing, loc, map);
        ...
        {
            if (slotGroup == null || !(slotGroup?.parent is ThingWithComps) || (slotGroup.parent as ThingWithComps).TryGetComp<CompDeepStorage>() == null) { return false; }
        ...
        {
            if (thing == null || !thing.Spawned || thing.Destroyed || thing.Map == null || thing.Position == IntVec3.Invalid)
            {
                // just in case
            #if DEBUG
                if (Utils._showDebug[(int) Dbf.TidyStacksOf])
                {
                    if (!thing.Spawned) { Log.Warning("Cannot tidy stack of unspawned " + thing); }
                    else if (thing.Destroyed) { Log.Warning("Cannot tidy stack of destroyed " + thing); }
                    else if (thing.Map == null) { Log.Warning("Cannot tidy stack on null map for " + thing); }
                    else if (thing.Position == IntVec3.Invalid) { Log.Warning("Cannot tidy invalid position for " + thing); }
                }
            #endif
                return;
            }
            ...
            var thingStacks = thing.Map.thingGrid.ThingsAt(thing.Position).Where(t => t.def == thing.def).ToList();
            var extra       = 0; // "extra" Sheep we pick up from wherever

            for (var i = 0; i < thingStacks.Count; i++)
            {
                ...
                {
                    // should never happen?
                    Log.Warning("LWM.DeepStorage found " + thingStacks[i].stackCount + " of " + thingStacks[i] + " when the stackLimit is " + stackLimit + ". This should never happen?");
                ...
                if (extra > 0)
                {
                    ...
                    var x = Math.Min(stackLimit - thingStacks[i].stackCount, extra);
                ...
                for (var j = thingStacks.Count - 1; j > i; j--)
                {
                    ...
                    {
                        // more than enough
                        var x = stackLimit - thingStacks[i].stackCount;
                        thingStacks[i].stackCount =  stackLimit;
            ...
            while (extra > 0)
            {
                ...
                var x = Math.Min(stackLimit, extra);
                extra -= x;
                var t = GenSpawn.Spawn(thing.def, thing.Position, thing.Map);
                t.stackCount = x;
            }
        ...
        {
            if (slotGroup?.parent is ThingWithComps thingWithComps && (compDeepStorage = thingWithComps.TryGetComp<CompDeepStorage>()) != null) { return true; }
        ...
        {
            if (Utils.HasDeepStorageComp(map?.haulDestinationManager?.SlotGroupAt(cell), out compDeepStorage)) { return true; }
        ...
        {
            if (Utils.GetDeepStorageOnCell(cell, map, out var comp))
            {
