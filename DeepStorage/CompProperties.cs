...
namespace LWM.DeepStorage
{
    public class Properties : CompProperties {
        public Properties() => this.compClass = typeof(LWM.DeepStorage.CompDeepStorage);
        ...
        private static StatCategoryDef _deepStorageCategory=null;
        public override IEnumerable<StatDrawEntry> SpecialDisplayStats(StatRequest req) {
            ...
            yield return new StatDrawEntry(Properties._deepStorageCategory, "LWM_DS_maxNumStacks".Translate().ToString(),
                                           (_size>1?
                                           "LWM_DS_TotalAndPerCell".Translate(_maxNumberStacks*_size,_maxNumberStacks).ToString()
                                           :_maxNumberStacks.ToString()),
                                           "LWM_DS_maxNumStacksDesc".Translate(),
                                           11 /*display priority*/);
            if (_minNumberStacks > 2)
            {
                yield return new StatDrawEntry(Properties._deepStorageCategory, "LWM_DS_minNumStacks".Translate(),
                    _size>1?
                        "LWM_DS_TotalAndPerCell".Translate(_minNumberStacks*_size,_minNumberStacks).ToString()
                        :_minNumberStacks.ToString(),
                    "LWM_DS_minNumStacksDesc".Translate(_minNumberStacks*_size),
                    10 /*display priority*/); //todo: more info here would be good!
            }

            if (_maxTotalMass > 0f)
            {
                yield return new StatDrawEntry(Properties._deepStorageCategory, "LWM_DS_maxTotalMass".Translate(),
                    _size>1?
                        "LWM_DS_TotalAndPerCell".Translate(Kg(_maxTotalMass*_size),Kg(_maxTotalMass)).ToString()
                        :Kg(_maxTotalMass),
                    "LWM_DS_maxTotalMassDesc".Translate(), 9);
            }

            if (_maxMassOfStoredItem > 0f)
            {
                yield return new StatDrawEntry(Properties._deepStorageCategory, "LWM_DS_maxMassOfStoredItem".Translate(),
                    Kg(_maxMassOfStoredItem),
                    "LWM_DS_maxMassOfStoredItemDesc".Translate(), 8);
            }

            if (AllowedCategoriesString!="")
            {
                yield return new StatDrawEntry(Properties._deepStorageCategory, "LWM_DS_allowedCategories".Translate(),
            ...
            {
                yield return new StatDrawEntry(Properties._deepStorageCategory, "LWM_DS_allowedDefs".Translate(),
                    AllowedDefsString,
            ...
            if (DisallowedString!="")
            {
                yield return new StatDrawEntry(Properties._deepStorageCategory, "LWM_DS_disallowedStuff".Translate(),
                    DisallowedString,
            ...
            get {
                if (_categoriesString==null) {
                    ...
                    foreach (var x in c) {
                        ...
                        _categoriesString+=DefDatabase<ThingCategoryDef>.GetNamed(x, true).LabelCap;
                    }
                }
        ...
        public string DisallowedString{
            get {
                if (_disallowedString==null) {
                    ...
                    if (!(c.NullOrEmpty())) {
                        foreach (var x in c) {
                            if (_disallowedString!="")
                            {
                                _disallowedString += "\n";
                            }
                            _disallowedString+=DefDatabase<ThingCategoryDef>.GetNamed(x, true).LabelCap;
                        }
                    }
        ...
        public int _size=0;
        public ThingDef _parent=null; // :p  Have to keep track of this myself

        private string _categoriesString=null; // for the Stats window (information window)
        private string _defsString=null;
        private string _disallowedString=null;
    }
