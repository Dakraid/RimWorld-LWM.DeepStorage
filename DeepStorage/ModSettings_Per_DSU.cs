...
#region
    ...
    public class DialogDSSettings : Window
    {
        ...
        private HashSet<ThingDef> _unitsToBeDisabled;
        ...
        {
            var contentRect      = new Rect(0, 0, inRect.width, inRect.height - (Window.CloseButSize.y + 10f)).ContractedBy(10f);
            var scrollBarVisible = _totalContentHeight > contentRect.height;
            var scrollViewTotal  = new Rect(0f, 0f, contentRect.width - (scrollBarVisible ? DialogDSSettings.ScrollBarWidthMargin : 0), _totalContentHeight);
            Widgets.DrawHighlight(contentRect);
            Widgets.BeginScrollView(contentRect, ref _scrollPosition, scrollViewTotal);
            var curY = 0f;
            var r    = new Rect(0, curY, scrollViewTotal.width, DialogDSSettings.LabelHeight);
            ...
            curY += DialogDSSettings.LabelHeight + 1f;

            if (!Settings._allowPerDsuSettings)
            {
                r = new Rect(5f, curY, scrollViewTotal.width - 10f, DialogDSSettings.LabelHeight);
                Widgets.Label(r, "LWMDSperDSUWarning".Translate());
                curY += DialogDSSettings.LabelHeight;
            }
            ...
            foreach (var u in Settings.AllDeepStorageUnits.ToList())
            {
                ...
                var isEnabled   = !_tracker.HasDefaultValueFor(u.defName, "def") && (_unitsToBeDisabled == null || !_unitsToBeDisabled.Contains(u));
                var wasEnabled  = isEnabled;
                var disableRect = new Rect(5f, curY, DialogDSSettings.LabelHeight, DialogDSSettings.LabelHeight);
                ...
                Widgets.Checkbox(disableRect.x, disableRect.y, ref isEnabled, DialogDSSettings.LabelHeight, false, true);
                ...
                if (!isEnabled && wasEnabled)
                {
                    ...
                    if (_unitsToBeDisabled == null) { _unitsToBeDisabled = new HashSet<ThingDef>(); }
                ...
                if (isEnabled && !wasEnabled)
                {
                    ...
                    if (_unitsToBeDisabled != null && _unitsToBeDisabled.Contains(u)) { _unitsToBeDisabled.Remove(u); }

                    if (_tracker.HasDefaultValueFor(u.defName, "def")) { _tracker.Remove(u.defName, "def"); }

                    if (!DefDatabase<ThingDef>.AllDefsListForReading.Contains(u)) { DialogDSSettings.ReturnDefToUse(u); }
                }
                //////////////// Select def: //////////////////
                r = new Rect(10f + DialogDSSettings.LabelHeight, curY, scrollViewTotal.width * 2 / 3 - 12f - DialogDSSettings.LabelHeight, DialogDSSettings.LabelHeight);
                ...
                // button clickiness:
                if (Widgets.ButtonInvisible(r)) { Find.WindowStack.Add(new DialogDsuSettings(u)); }
                //////////////// Reset button: //////////////////
                r = new Rect(scrollViewTotal.width * 2 / 3 + 2f, curY, scrollViewTotal.width / 3 - 7f, DialogDSSettings.LabelHeight);

                if (_tracker.IsChanged(u.defName) && Widgets.ButtonText(r, "ResetBinding".Translate())) { DialogDSSettings.ResetDsuToDefaults(u.defName); }
                curY += DialogDSSettings.LabelHeight + 2f;
            }
            ...
            // close button:
            r = new Rect(inRect.width / 2 - Window.CloseButSize.x / 2, inRect.height - Window.CloseButSize.y - 5f, Window.CloseButSize.x, Window.CloseButSize.y);
            ...
            if (Widgets.ButtonText(r, "CloseButton".Translate()))
            {
                if (_unitsToBeDisabled != null && _unitsToBeDisabled.Count > 0)
                {
                    ...
                    foreach (var d in _unitsToBeDisabled)
                    {
                        ...
                        DialogDSSettings.RemoveDefFromUse(d);
            ...
            r = new Rect(10f, inRect.height - Window.CloseButSize.y - 5f, 2 * Window.CloseButSize.x, Window.CloseButSize.y);
            ...
            if (_tracker.HasAnyDefaultValues && Widgets.ButtonText(r, "LWM.ResetAllToDefault".Translate()))
            {
                ...
                DialogDSSettings.ResetAllToDefaults();
            }
        ...
        private static void RemoveDefFromUse(ThingDef def)
        {
            ...
            //   #reflection #magic
            typeof(DefDatabase<>).MakeGenericType(typeof(ThingDef)).GetMethod("Remove", BindingFlags.Static | BindingFlags.NonPublic).Invoke(
                null, new object[]
                {
                    def
                }
            ...
            if (def.designationCategory != null)
            {
                ...
                def.designationCategory.AllResolvedDesignators.RemoveAll(x => x is Designator_Build && ((Designator_Build) x).PlacingDef == def);
            }
        }
        ...
        private static void ReturnDefToUse(ThingDef def)
        {
            ...
            if (def.designationCategory != null) { def.designationCategory.AllResolvedDesignators.Add(new Designator_Build(def)); }
        }
        ...
        private static void ResetDsuToDefaults(string resetDefName)
        {
            ...
            while (resetAll && Settings._defTracker.GetFirstDefaultValue(out defName, out prop, out tmpObject) || Settings._defTracker.GetFirstDefaultValueFor(defName, out prop, out tmpObject))
            {
                ...
                if (def == null)
                {
                    var tmp = Settings._defTracker.GetDefaultValue(defName, "def", def);

                    if (tmp != null)
                    {
                        def = tmp;
                        // We are resetting the def, so we need it back in the DefDatabase!
                        DialogDSSettings.ReturnDefToUse(def);
                ...
                if (prop == "label") { def.label                                                                 = (string) tmpObject; }
                else if (prop == "maxNumStacks") { def.GetCompProperties<Properties>()._maxNumberStacks          = (int) tmpObject; }
                else if (prop == "maxTotalMass") { def.GetCompProperties<Properties>()._maxTotalMass             = (float) tmpObject; }
                else if (prop == "maxMassStoredItem") { def.GetCompProperties<Properties>()._maxMassOfStoredItem = (float) tmpObject; }
                else if (prop == "showContents") { def.GetCompProperties<Properties>()._showContents             = (bool) tmpObject; }
                else if (prop == "storagePriority") { def.building.defaultStorageSettings.Priority               = (StoragePriority) tmpObject; }
                else if (prop == "overlayType") { def.GetCompProperties<Properties>()._overlayType               = (GuiOverlayType) tmpObject; }
                else if (prop == "filter") { def.building.fixedStorageSettings.filter                            = (ThingFilter) tmpObject; }
                else if (prop == "def")
                {
                ...
                else { Log.Error("LWM.DeepStorage: FAILED TO RESET OPTION TO DEFAULT: " + defName + ", " + prop); }
                Settings._defTracker.Remove(defName, prop);
        ...
        public static void ExposeDsuSettings(IEnumerable<ThingDef> units)
        {
            ...
            if (units == null)
            {
                Log.Warning("Passed null units");

                return;
            }

            if (Settings._defTracker == null)
            {
                Log.Error("DefChangeTracker is null");
            ...
            foreach (var u in units.ToList())
            {
                ...
                Settings._defTracker.ExposeSetting(defName, "label", ref u.label);
                ...
                Settings._defTracker.ExposeSetting(defName, "storagePriority", ref tmpSp);
                ...
                if (u.building?.fixedStorageSettings != null) { Settings._defTracker.ExposeSettingDeep(defName, "filter", ref u.building.fixedStorageSettings.filter); }
                ...
                if (Scribe.mode == LoadSaveMode.LoadingVars)
                {
                    ...
                    var disabled = false;
                    Scribe_Values.Look(ref disabled, "DSU_" + defName + "_disabled");
                    ...
                    if (disabled)
                    {
                        ...
                        DialogDSSettings.RemoveDefFromUse(u);
                    }
                }
        ...
        private class DialogDsuSettings : Window
        {
            ...
            public DialogDsuSettings(ThingDef def)
            {
                ...
                if (_tracker.HasDefaultValueFor(def.defName, "filter")) { _useCustomThingFilter = true; }
            ...
            {
                var cp = _def.GetCompProperties<Properties>();

                if (_tmpLabel != _def.label) { return false; }

                if (_tmpMaxMassStoredItem != cp._maxMassOfStoredItem || _tmpMaxNumStacks != cp._maxNumberStacks || _tmpMaxTotalMass != cp._maxTotalMass || _tmpOverlayType != cp._overlayType || _tmpShowContents != cp._showContents)
                {
                    return false;
                }

                if (_tmpStoragePriority != _def.building.defaultStorageSettings.Priority) { return false; }

                if (_useCustomThingFilter) { return false; }
            ...
            public override void DoWindowContents(Rect inRect) // For a specific DSU
            {
                ...
                var s = new Rect(inRect.x, inRect.y, inRect.width, inRect.height - Window.CloseButSize.y - 5f);
                ...
                var inner = new Rect(0, 0, s.width - 20, _y);
                ...
                l.TextFieldNumericLabeled(
                    "LWM_DS_maxNumStacks".Translate().CapitalizeFirst() + " " + "LWM_DS_Default".Translate(_tracker.GetDefaultValue(_def.defName, "maxNumStacks", _tmpMaxNumStacks)), ref _tmpMaxNumStacks, ref tmpstring
                );
                tmpstring = null;

                l.TextFieldNumericLabeled(
                    "LWM_DS_maxTotalMass".Translate().CapitalizeFirst() + " " + "LWM_DS_Default".Translate(_tracker.GetDefaultValue(_def.defName, "maxTotalMass", _tmpMaxTotalMass).ToString()), ref _tmpMaxTotalMass, ref tmpstring
                );
                tmpstring = null;

                l.TextFieldNumericLabeled(
                    "LWM_DS_maxMassOfStoredItem".Translate().CapitalizeFirst() + " " + "LWM_DS_Default".Translate(_tracker.GetDefaultValue(_def.defName, "maxMassStoredItem", _tmpMaxMassStoredItem).ToString()), ref _tmpMaxMassStoredItem,
                    ref tmpstring
                );
                ...
                l.GapLine();
                l.CheckboxLabeled("LWMDSpDSUchangeFilterQ".Translate(), ref _useCustomThingFilter, "LWMDSpDSUchangeFilterQDesc".Translate());
                _y = l.CurHeight;
                l.End();
                ...
                if (_useCustomThingFilter)
                {
                    ...
                    var r = new Rect(20, _y, (inner.width - 40) * 3 / 4, DialogDsuSettings.CustomThingFilterHeight);
                    _y += DialogDsuSettings.CustomThingFilterHeight;
                    ThingFilterUI.DoThingFilterConfigWindow(r, _thingFilterState, _customThingFilter);
                }
                else
                {
                    // not using custom thing filter:
                    if (_customThingFilter != null || _tracker.HasDefaultValueFor(_def.defName, "filter"))
                    {
                        _customThingFilter = null;
                        ...
                        if (_tracker.HasDefaultValueFor(_def.defName, "filter"))
                        {
                            ...
                            _def.building.fixedStorageSettings.filter = _tracker.GetDefaultValue<ThingFilter>(_def.defName, "filter", null);
                ...
                // Cancel button
                var closeRect = new Rect(inRect.width - Window.CloseButSize.x, inRect.height - Window.CloseButSize.y, Window.CloseButSize.x, Window.CloseButSize.y);
                ...
                closeRect = new Rect(inRect.width - (2 * Window.CloseButSize.x + 5f), inRect.height - Window.CloseButSize.y, Window.CloseButSize.x, Window.CloseButSize.y);
                ...
                if (Widgets.ButtonText(closeRect, "AcceptButton".Translate()))
                {
                    ...
                    _tracker.UpdateToNewValue(_def.defName, "maxNumStacks", _tmpMaxNumStacks, ref props._maxNumberStacks);
                    _tracker.UpdateToNewValue(_def.defName, "maxTotalMass", _tmpMaxTotalMass, ref props._maxTotalMass);
                    _tracker.UpdateToNewValue(_def.defName, "maxMassStoredItem", _tmpMaxMassStoredItem, ref props._maxMassOfStoredItem);
                    _tracker.UpdateToNewValue(_def.defName, "showContents", _tmpShowContents, ref props._showContents);
                    _tracker.UpdateToNewValue(_def.defName, "overlayType", _tmpOverlayType, ref props._overlayType);
                    ...
                    if (_useCustomThingFilter)
                    {
                        // danger ahead - automatically use it, even if stupidly set up
                        if (!_tracker.HasDefaultValueFor(_def.defName, "filter"))
                        {
                ...
                // Reset to Defaults
                closeRect = new Rect(inRect.width - (4 * Window.CloseButSize.x + 10f), inRect.height - Window.CloseButSize.y, 2 * Window.CloseButSize.x, Window.CloseButSize.y);
